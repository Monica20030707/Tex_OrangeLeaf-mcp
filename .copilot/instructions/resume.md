...
3.  **Template Integrity & Content Selection:**
    ...
    -   **Smart Tailoring:** For the selected items, look up their corresponding YAML file in `knowledge/`. 
        ...
        -   If the job title in the description is close but not identical to a resume title (for example, "backend engineer" vs. "full stack software engineer"), tweak the resume wording to reflect the job title while preserving the same seniority level (intern, associate, junior, etc.). Do not change the level of proficiency. Treat related engineering titles as equivalent when appropriate, including software engineer, software developer, full stack engineer, full stack developer, backend engineer, and cloud engineer.
